namespace Reservations.Services
{
    ...
    public class ReservationsService : IReservationsService
    {
        ...
        public IEnumerable<ReservationItem> GetByDay(DateTime day, int hallNumber)
        {
            ...
            reservations = _queryAll.Execute().Where(x => x.Hall.Number == hallNumber).OrderBy(x => x.From).ToList();
            foreach (var item in reservations)
            {
                if (!_queryAllLectureHalls.Execute().Any(x => x.Number == item.Hall.Number))
                {
                    throw new ArgumentException("Hall does not exist in the collection for the selected day");
                }
            }
            //return Mapper.Map<ReservationItem>(reservations);
        ...
        public IEnumerable<HallFreeHoursStatisticsItem> GetHallsFreeHoursByDay(DateTime day)
        {
            ...
            List <HallFreeHoursStatisticsItem> freeHoursStat = new List<HallFreeHoursStatisticsItem>();
            var reservationItems = _queryAll.Execute().ToList();
            double freeHours = 0;
            foreach (var item in reservationItems)
            {
                if (_queryAllLectureHalls.Execute().Any(x => x.Number == item.Hall.Number))
                {
                   freeHours = (item.To.Subtract(item.From).TotalHours);
                    freeHoursStat.Add(new HallFreeHoursStatisticsItem { FreeHoursNumber = (int)freeHours, HallNumber = item.Hall.Number});
                }
            }
            //return (int)freeHours;
            //return new List<HallFreeHoursStatisticsItem>();
            return freeHoursStat;
        }
    }
}